...
type FacadeConfig struct {
	...
	GenerateCommandsTopic CommandTopicGenerator
	...
	//
	// It allows you to use topic per command or one topic for every command. [todo - add to doc]
	GenerateEventsTopic EventTopicGenerator
	...
	// Router handlers will be automatically generated by RouterHandlerFunc of Command and Event handlers.
...
type Facade struct {
	commandsTopic CommandTopicGenerator
	commandBus    *CommandBus

	eventsTopic EventTopicGenerator
	eventBus    *EventBus
...
func NewFacade(config FacadeConfig) (*Facade, error) {
	...
	if config.CommandsEnabled() {
		c.commandBus = NewCommandBus(config.CommandsPublisher, config.GenerateCommandsTopic, config.CommandEventMarshaler)
	} else {
		config.Logger.Info("Empty GenerateCommandsTopic, command bus will be not created", nil)
	}
	if config.EventsEnabled() {
		c.eventBus = NewEventBus(config.EventsPublisher, config.GenerateEventsTopic, config.CommandEventMarshaler)
	} else {
