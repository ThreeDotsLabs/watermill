...
By default, it is just a topic name (`TopicSubscriptionName`).

When you want to consume topic by multiple subscribers, you should use
`TopicSubscriptionNameWithSuffix` or your custom function to generate subscription name.

#### Connecting

By default, Google Cloud Pub/Sub will connect to "local" Pub/Sub available in production environment.

For development, you can use Docker image with the emulator ([check the our Getting Started guide]({{< ref "getting-started#subscribing_gcloud" >}})).
